...
def collate_fn_wl2(batch, input_dim, start_idx_all, end_idx_all, device):
    all_data_dicts = []
    nICs = len(start_idx_all)
    for i, st, ed in zip(range(nICs), start_idx_all, end_idx_all):
        
        data = torch.zeros([len(batch), 1, batch[0][0].shape[2]]).to(device)
        target = torch.zeros([len(batch), ed - st + 1, input_dim]).to(device)
    
        for b, (snap, ts) in enumerate(batch):
            data[b, 0, :] = snap[:, st, :]
            target[b, :, :] = snap[:, st:(ed + 1), :input_dim]
...
def parse_datasets2(raw_datafile, sim_datafile, param_datafile, args, device):
    """
    Take raw_data i.e. WL tensors, sim data and flux rope parameter information to construct dataloader objects for train, val and test sets depending on the chosen training mode i.e. 2-4 sims. The difference from `parse_datasets` lies mainly in the time scaling and in accomodation of variable length sequences - the latter can by done by combination of removing hardcoding from tMin and splitting data into multiple initial conditions. We will focus on splitting the data properly.
    """
    data = np.load(raw_datafile)
    files = np.load(sim_datafile)
    params = np.loadtxt(param_datafile)
    ...
    if args["test_mode"]=="t2p1":
        ...
        train_idx = np.array([0, 2, 4, 6, 8, 10, 11, 12, 13, 18, 19, 20])
        val_idx = np.array([5, 9, 16, 21])
        test_idx = np.array([1, 3, 7, 14, 15, 17, 22])
    ...
    tMaxIdx = len(all_timesteps) - 1
    nTimesteps = tMaxIdx - tMinIdx + 1
    tMaxVal = 180 # we have data up to 180 minutes.
    ...
    valid_indices = list(range(nTimesteps - args["steps_to_predict"]))
    
    start_idx_all = sorted(random.sample(valid_indices, args["sampled_ics"]))
    
    end_idx_all = []
    for idx in start_idx_all:
        end_idx = min(idx + args["steps_to_predict"] + 1, nTimesteps)
    ...
    train_dataloader = DataLoader(train_dataset, 
                              ...
                              shuffle=False,
                            #   num_workers=1, 
                              collate_fn = lambda batch: collate_fn_wl2(batch, input_dim, start_idx_all, end_idx_all, device))
...
def parse_datasets_randomized(raw_datafile, sim_datafile, param_datafile, args, device):
    """
    When training NODE on a larger dataset, use random splits of data based on train, validation and test fractions. Restrict train dataloader batch size to 4, while the remaining dataloaders will load their data all at once. Currently used for processing 120 runs of CR2192.
    """
