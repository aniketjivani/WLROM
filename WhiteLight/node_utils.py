...
class PNODE_Conv(nn.Module):
    def __init__(self, 
        ...
        init_network_weights_xavier_normal(self.encoder)
        init_network_weights_xavier_normal(self.decoder_mlp)
        init_network_weights_xavier_normal(self.decoder_conv)
...
class VPNODE_Conv(nn.Module):
    def __init__(self, 
                 ...
                 dropout=False,
                 resize_dims=(32, 128)):
        super(VPNODE_Conv, self).__init__()
        ...
        self.fc_mu = nn.Sequential(nn.Linear(64, latent_dim),
                                   nn.Tanh())
        self.fc_log_var = nn.Sequential(nn.Linear(64, latent_dim),
                                        nn.ELU())
        ...
        self.decoder_mlp = decoder_mlp
        self.decoder_conv = decoder_conv
        self.softplus_act = torch.nn.Softplus(beta=0.5)
        
        init_network_weights_xavier_normal(self.encoder)
        init_network_weights_xavier_normal(self.decoder_mlp)
        init_network_weights_xavier_normal(self.decoder_conv)
        init_network_weights_xavier_normal(self.fc_mu)
        init_network_weights_xavier_normal(self.fc_log_var)
        
        
    # def reparametrize(self, mu, log_var):
    def reparametrize(self, mu, log_var):
        # std = torch.log(1 + torch.exp(0.5 * log_var))
        # std = torch.exp(0.5 * log_var)
        std = self.softplus_act(log_var)
    ...
    def forward(self, t, y):
        ...
        nbatch, ntraj, nseq = init_state.shape
        init_state = init_state.reshape((nbatch * ntraj, 1, self.resize_dims[0], self.resize_dims[1]))
        init_state = self.encoder(init_state)
        init_latent_mu = self.fc_mu(init_state)
        init_latent_var = self.fc_log_var(init_state)
        # init_latent_std = self.softplus_act(init_latent_var)
        init_latent = self.reparametrize(init_latent_mu, init_latent_var).reshape((nbatch, ntraj, self.latent_dim))
        init_latent = torch.cat((init_latent, y[:, :, self.input_dim:]),-1)
        latent_states = odeint(self.pnode, init_latent, t)
        ls_init = latent_states[:, :, :, :self.latent_dim]
        ntraj2, nbatch2, nseq2, _  = ls_init.shape
        ...
        return init_latent_mu, init_latent_var, pred_sol
